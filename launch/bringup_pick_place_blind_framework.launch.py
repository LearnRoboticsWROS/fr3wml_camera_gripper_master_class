...
def generate_launch_description():
    ...
    scene_file_arg = DeclareLaunchArgument(
        ...
        description="Path to the scene loader YAML file"
    )

    params_file_arg = DeclareLaunchArgument(
        "params_file",
        default_value=default_params_file,
        description="Path to the executor framework YAML file"
    )
    ...
    scene_loader_node = Node(
        ...
        executable="scene_loader_bottle_capsule",
        name="scene_loader_node",
        output="screen",
    ...
    executor_node = Node(
        package="fr3wml_camera_gripper",
        executable="pick_place_bottle_capsule_framework",
        name="pick_place_bottle_capsule_framework",
        output="screen",
    ...
    delayed_scene_loader = TimerAction(
        period=5.0,
        actions=[scene_loader_node]
    )

    delayed_executor = TimerAction(
        period=8.0,
        actions=[executor_node]
    )
